 ...
 * memory allocation, data movement, etc.
 Devesh Jaiswal DJ1380
 Compiled with: gcc -Wall -std=c99 -fopenmp -o heatdist heatdist.c -lm
 Ran With: ./heatdist x y z k
 */
...
void  parallel_heat_dist(float * playground, unsigned int N, unsigned int iterations)
{
    ...
    //allocates the temporary array and holds updated values
      ...
      //iterate over fixed # of iterations
      for (k = 0; k < iterations; k++)
      {
          //parallelize over i and j calculates averages and updates cell in temporary array
          //each thread has its own i and j but of course temporary and playfround is shared
        #pragma omp parallel for private(i, j) shared(playground, temp)
        for (i = 1; i < upper; i++)
        {
          for (j = 1; j < upper; j++)
          {
        ...
        //copy updated values back to original
        memcpy((void *)playground, (void *)temp, num_bytes);
      }
      //free the memory
      free(temp);
